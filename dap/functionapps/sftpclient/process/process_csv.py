...
def process_csv(
    ...
    """
    Process csv and write to blob
    """
    try:
        activity_type = ActivityTypes.PROCESS_CSV.value
        logger.info("Started %s activity for csv file: %s", activity_type, file_name)
        ...
        file_name_new = f"{os.path.splitext(file_name)[0]}_{timestamp}.csv"
        ...
        if not logging_completed:
            error_string = (
                f"Unable to process the csv file: {file_name}. An error occurred: {e}"
            )
