...
from validations.utils import (
    validate_excel_empty_check,
    file_name_validation_l2,
    validate_excel_file,
    validate_excel_file_encoding,
)
...
def execute_validations_excel(
    ...
    """
    validations for EXCEL
    """
    try:
        activity_type = ActivityTypes.VALIDATIONS.value
        logger.info(
            "Started %s activity for excel file: %s", activity_type, source_file_name
        )
        ...
        validations = [
            ...
            (
                validate_excel_empty_check,
                (source_blob_size, fd_sample_b_io, source_file_name),
            ),
            (validate_excel_file_encoding, (fd_sample_b_io, source_file_name)),
            (validate_excel_file, (fd_sample_b_io, source_file_name)),
        ]
        ...
        file_pattern_name = results["file_name_validation_l2"]
        ...
        logger.info(
            "Completed %s activity for excel file: %s", activity_type, source_file_name
        )
        ...
        error_string = (
            f"Unable to execute validations for Excel {source_file_name}: {e}"
        )
        handle_logging_error(
            activity_run_id,
            activity_type,
